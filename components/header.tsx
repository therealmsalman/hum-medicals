...
import { Check, LogOut, Menu, Moon, Sparkles, Sun, UserRound, X } from 'lucide-react';
...
export function Header() {
  ...
  useEffect(() => {
    checkAuth();
    window.addEventListener('hum-medicals:auth-change', checkAuth);
    return () => window.removeEventListener('hum-medicals:auth-change', checkAuth);
  }, [pathname]);
          ...
          {user ? (
            <div className="hidden items-center gap-1.5 sm:flex">
              ...
              <button
                ...
                className="rounded-xl border border-slate-200/80 p-2 text-slate-500 hover:border-red-300 hover:bg-red-50 hover:text-red-600 dark:border-slate-800 dark:text-slate-400 dark:hover:border-red-900 dark:hover:bg-red-950/40 dark:hover:text-red-400 transition-all"
                aria-label="Sign out"
              >
                <LogOut size={15} />
              </button>
      ...
      {open && (
        <div className="border-t border-slate-200/80 bg-white/95 px-5 py-5 backdrop-blur-xl dark:border-slate-800 dark:bg-[#090d16]/95 lg:hidden">
          ...
          <div className="mt-4 border-t border-slate-200/80 pt-4 dark:border-slate-800">
            {user ? (
              <div className="flex items-center justify-between py-2 border-b border-slate-200/60 pb-3 mb-2 dark:border-slate-800">
                <Link
                  href="/account"
                  onClick={() => setOpen(false)}
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-semibold text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/40"
                >
