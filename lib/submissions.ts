...
export type Submission={id:string;authorId:string;authorName:string;authorEmail:string;title:string;type:string;abstract:string;status:'Received';createdAt:string};
...
export async function createSubmission(input:Omit<Submission,'id'|'status'|'createdAt'>){
  const submission:Submission={...input,id:crypto.randomUUID(),status:'Received',createdAt:new Date().toISOString()};
