...
export async function POST(request:Request){
  ...
  if(!key)return NextResponse.json({message:'AI Tutor is not configured. Add GEMINI_API_KEY to this Vercel project’s Production environment, then redeploy.'},{status:503});
  let input:{question?:string;context?:string};
  try{input=await request.json();}catch{return NextResponse.json({message:'Invalid request.'},{status:400});}
  const question=input.question?.trim();
  const context=input.context?.trim().slice(0,5000)||'';
  ...
  try{
    ...
    if(!response.ok)return NextResponse.json({message:payload?.error?.message||'Gemini could not answer this question.'},{status:response.status});
  ...
  }catch{return NextResponse.json({message:'Unable to reach the AI Tutor. Please try again.'},{status:502});}
}
