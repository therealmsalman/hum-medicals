...
export async function POST(request:Request){
  try{
    ...
    const {title,type,abstract,manuscript,consent}=await request.json();
    ...
    const submission=await createSubmission({authorId:user.id,authorName:user.name,authorEmail:user.email,title:title.trim(),type,abstract:abstract.trim(),manuscript:manuscript.trim()});
