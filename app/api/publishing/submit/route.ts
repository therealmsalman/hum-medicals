...
export async function POST(request:Request){
  try{
    ...
    const {title,type,abstract,consent}=await request.json();
    ...
    const submission=await createSubmission({authorId:user.id,authorName:user.name,authorEmail:user.email,title:title.trim(),type,abstract:abstract.trim()});
    return NextResponse.json({submission:{id:submission.id,status:submission.status,createdAt:submission.createdAt},message:`Submission received. Your reference is ${submission.id}.`},{status:201});
