  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="max-w-3xl">
        <p className="eyebrow">
          <BookOpen size={13} className="text-teal-600 dark:text-teal-400" /> Peer-Reviewed Education
        </p>
        ...
        <p className="mt-3 text-sm leading-relaxed text-slate-600 dark:text-slate-300 sm:text-base">
          150 structured research summaries, evidence syntheses, and clinical guides authored to bridge bedside decision-making with modern cardiology literature.
        </p>
      </div>

      {/* Community Approved Submissions */}
      <div className="mt-12">
        <ApprovedSubmissions collection="paper" />
      </div>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by diagnosis, clinical syndrome, author, or keyword…"
      ...
      <div className="mt-6 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
        <span>
          Showing {visible.length} of {list.length} publication{list.length !== 1 ? 's' : ''}
        </span>
        <span>
          Page {page} of {pages}
        </span>
      ...
      {/* Pagination */}
      {pages > 1 && (
        <nav aria-label="Publication pages" className="mt-12 flex flex-wrap items-center justify-center gap-2">
          <button
            disabled={page === 1}
